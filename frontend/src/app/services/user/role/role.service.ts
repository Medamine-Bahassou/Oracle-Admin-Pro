...
export class RoleService {
  ...
  assignRoleToUser(username: string, role: string): Observable<any> {
    return this.http.post(`${this.apiUrl}/assign`, {username, role})
      .pipe(
        catchError(this.handleError)
      );
  }
  createRoleWithPrivileges(roleName: string, privileges: string[]): Observable<any> {
    return this.http.post(`${this.apiUrl}/create`, {roleName, privileges})
      .pipe(
        catchError(this.handleError)
      );
  }
  ...
  dropRole(roleName: string): Observable<any> {
    return this.http.delete(`${this.apiUrl}/drop?roleName=${roleName}`)
      .pipe(
        catchError(this.handleError)
      );
  }
